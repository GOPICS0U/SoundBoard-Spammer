...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'play') {
        ...
        if (!channel.isVoiceBased()) {
            return interaction.reply({ content: '❌ Le salon choisi doit être vocal.', flags: 64 });
        }

        const filePath = `./sounds/${soundName}.mp3`;
        if (!fs.existsSync(filePath)) {
            return interaction.reply({ content: `❌ Le fichier ${soundName}.mp3 n'existe pas.`, flags: 64 });
        }
    ...
    if (interaction.commandName === 'addsound') {
        ...
        if (!file.name.endsWith('.mp3') && !file.name.endsWith('.wav')) {
            return interaction.reply({ content: '❌ Seuls les fichiers `.mp3` ou `.wav` sont acceptés.', flags: 64 });
        }

        const savePath = `./sounds/${name}.mp3`; // tout est stocké en .mp3
        ...
        try {
            ...
            console.error(err);
            await interaction.reply({ content: '❌ Erreur lors du téléchargement du fichier.', flags: 64 });
        }
    }
});
